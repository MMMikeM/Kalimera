import { sqliteTable, foreignKey, primaryKey, index, uniqueIndex, integer, text, real } from "drizzle-orm/sqlite-core"
...
(table) => [index("idx_tags_section_order").on(table.section, table.sectionDisplayOrder),
...
export const practiceAttempts = sqliteTable("practice_attempts", {
	...
	attemptedAt: integer("attempted_at").default(sql`unixepoch()`).notNull(),
	drillId: text("drill_id"),
},
(table) => [index("idx_practice_attempts_drill").on(table.userId, table.drillId, table.attemptedAt),
...
export const verbDetails = sqliteTable("verb_details", {
	vocabId: integer("vocab_id").primaryKey().references(() => vocabulary.id, { onDelete: "cascade" } ),
...
export const vocabularyTags = sqliteTable("vocabulary_tags", {
	vocabularyId: integer("vocabulary_id").notNull().references(() => vocabulary.id, { onDelete: "cascade" } ),
	tagId: integer("tag_id").notNull().references(() => tags.id, { onDelete: "cascade" } ),
	displayOrder: integer("display_order"),
},
(table) => [index("idx_vocabulary_tags_tag_order").on(table.tagId, table.displayOrder),
...
export const verbConjugations = sqliteTable("verb_conjugations", {
	id: integer().primaryKey({ autoIncrement: true }),
	vocabId: integer("vocab_id").notNull().references(() => vocabulary.id, { onDelete: "cascade" } ),
	tense: text().notNull(),
	person: text().notNull(),
	form: text().notNull(),
	stem: text(),
...
(table) => [uniqueIndex("idx_verb_conjugations_unique").on(table.vocabId, table.tense, table.person),
index("idx_verb_conjugations_vocab_tense").on(table.vocabId, table.tense),
index("idx_verb_conjugations_vocab").on(table.vocabId),
]);

export const verbImperatives = sqliteTable("verb_imperatives", {
	id: integer().primaryKey({ autoIncrement: true }),
	vocabId: integer("vocab_id").notNull().references(() => vocabulary.id, { onDelete: "cascade" } ),
	aspect: text().notNull(),
	number: text().notNull(),
...
(table) => [uniqueIndex("idx_verb_imperatives_unique").on(table.vocabId, table.aspect, table.number),
...
(table) => [index("idx_notification_logs_sent_at").on(table.sentAt),
index("idx_notification_logs_user").on(table.userId),
]);

export const nominalForms = sqliteTable("nominal_forms", {
	id: integer().primaryKey({ autoIncrement: true }),
	vocabId: integer("vocab_id").notNull().references(() => vocabulary.id, { onDelete: "cascade" } ),
...
(table) => [uniqueIndex("idx_nominal_forms_unique").on(table.vocabId, table.grammaticalCase, table.number, table.genderKey),
...
(table) => [uniqueIndex("idx_vocabulary_greek_text").on(table.greekText),
...
export const vocabDailyResults = sqliteTable("vocab_daily_results", {
	userId: integer("user_id").notNull().references(() => users.id, { onDelete: "cascade" } ),
	vocabId: integer("vocab_id").notNull().references(() => vocabulary.id, { onDelete: "cascade" } ),
...
(table) => [index("idx_vocab_daily_user_drill").on(table.userId, table.drillId),
primaryKey({ columns: [table.userId, table.vocabId, table.drillId, table.practicedDate], name: "vocab_daily_results_pk"}),
]);

export const vocabMastery = sqliteTable("vocab_mastery", {
	userId: integer("user_id").notNull().references(() => users.id, { onDelete: "cascade" } ),
	vocabId: integer("vocab_id").notNull().references(() => vocabulary.id, { onDelete: "cascade" } ),
	drillId: text("drill_id").notNull(),
...
(table) => [index("idx_vocab_mastery_review").on(table.userId, table.drillId, table.nextReviewAt),
...
(table) => [index("idx_vocabulary_reviews_user_review").on(table.userId, table.nextReviewAt),
