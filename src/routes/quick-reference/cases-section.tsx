...
import { Card, MonoText, SectionHeading, KeyInsight, CollapsibleSection } from "../../components";
...
export const CasesSection: React.FC = () => (
	<section id="cases" className="space-y-6">
		...
		<Alert variant="info">
			...
			<AlertDescription>
				Greek uses cases to show a word's role. Ask these questions and you'll know which case to use.
			</AlertDescription>
		</Alert>
					...
					<div key={index} className="p-4 bg-white rounded-xl shadow-sm border border-honey-300">
						...
						<div className="space-y-1">
							{check.examples.map((example, i) => (
								<MonoText key={i} variant="greek" size="sm" className="block text-honey-text">{example}</MonoText>
							))}
						</div>
					</div>
				))}
