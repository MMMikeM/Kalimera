...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn('status', 'varchar', (col) => col.notNull().defaultTo('unprocessed'))
    .addColumn('difficulty_level', 'integer', (col) => col.notNull().defaultTo(0))
    .addColumn('is_problem_word', 'boolean', (col) => col.notNull().defaultTo(false))
  ...
  await db.schema
    ...
    .addColumn('vocab_id', 'integer', (col) => col.primaryKey().references('vocabulary.id').onDelete('cascade'))
  ...
  await db.schema
    ...
    .addColumn('id', 'serial', (col) => col.primaryKey())
    .addColumn('verb_details_id', 'integer', (col) => col.notNull().references('verb_details.vocab_id').onDelete('cascade'))
  ...
  await db.schema
    ...
    .addColumn('last_mistake_at', 'timestamp', (col) => col.notNull().defaultTo(sql`now()`))
  ...
  await db.schema
    ...
    .addColumn('id', 'serial', (col) => col.primaryKey())
    .addColumn('session_id', 'integer', (col) => col.references('practice_sessions.id').onDelete('cascade'))
    .addColumn('vocabulary_id', 'integer', (col) => col.references('vocabulary.id').onDelete('cascade'))
    ...
    .addColumn('attempted_at', 'timestamp', (col) => col.notNull().defaultTo(sql`now()`))
    .execute()

  // Indexes for performance
  await db.schema.createIndex('idx_vocabulary_category').on('vocabulary').column('category').execute()
  await db.schema.createIndex('idx_vocabulary_word_type').on('vocabulary').column('word_type').execute()
  await db.schema.createIndex('idx_vocabulary_difficulty').on('vocabulary').column('difficulty_level').execute()
  await db.schema.createIndex('idx_vocabulary_problems').on('vocabulary').column('is_problem_word').where('is_problem_word', '=', true).execute()
